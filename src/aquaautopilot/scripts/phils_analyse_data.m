if (0)
    datas = csvread('aqua_state_14_11.csv',2);
    datadepth = csvread('filtered_depth_14_11.csv',2);
    datatarget = csvread('target_pose_14_11.csv',2);
    ...
    FirstTime = datas(1,1);    set(hdl,'FontName','Courier','FontSize',15);
...
xlim(Limits);
xlabel('Time (s)');
line(Limits,[-20 -20],'Color','r');
line(Limits,[20 20],'Color','r');
text(Limits(2)+0.2,-20,'-1','Color','r');
text(Limits(2)+0.2,20,'1','Color','r');
text(Limits(2)+0.2,0,{'Yaw Unitless' 'Command'},'Color','r','FontSize',10,'FontName','Courier');
...
if (0)
    ...
    hk(2) = plot(DepthTime,-200*CorrectedDepth+5700,'g-');text(Limits(2)+0.2,20,'1','Color','r');
