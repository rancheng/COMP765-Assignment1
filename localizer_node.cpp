...
public:
  ...
  Localizer( int argc, char** argv )
  {
    ...
    else // Localization starting at 0,0 initialization
    {
      ...
      map_center_x = localization_line_image.size().width/2;
      map_center_y = localization_line_image.size().height/2;

      // Initialize particles around the (0,0)
      std::normal_location_dis<double> location_dis(0.0,10.0);
      std::normal_location_dis<double> heading_dis(0.0,0.26);
  ...
  void adjustDrawParticles()
  {
    localization_result_image = localization_line_image.clone();
    for(int i = 0; i < MAX_PARTICLES; i++)
    {
      double x = adjustXPixels(particles[i].x, particles[i].yaw);
      double y = adjustYPixels(particles[i].y, particles[i].yaw);
  ...
  void updateParticlesWithObservation(double lastX, double lastY)
  {
    cv::Mat cam_image = current_camera_image.clone();
    ...
    for(int i = 0; i < MAX_PARTICLES; i++)
    {
      ...
      double posX = 1-1/2*(std::abs(map_center_x - particles[i].x - lastX + 1));
      double posY = 1-1/2*(std::abs(map_center_y - particles[i].y - lastY + 1));
  ...
  // Propagates the pixels based on the map_center_x and estimated_robo_image_y pixels
  ...
  {
    std::normal_location_dis<double> location_dis(0.0,10.0);
    std::normal_location_dis<double> heading_dis(0.0,0.26); //-15 to +15 degrees
    std::exponential_location_dis<double> exp_location_dis(0.1);
    ...
    { 
      ...
      particles[i].x = oldParticles[index].x + std::roundl(location_dis(rand_num_gen)) + (guessX);
      particles[i].y = oldParticles[index].y + std::roundl(location_dis(rand_num_gen)) + (guessY);
      particles[i].yaw = heading_dis(rand_num_gen);
  ...
  void drawBelief()
  {
    map_center_x = particles[0].x;
    map_center_y = particles[0].y;
    double radius = 15.0;
    cv::circle(localization_result_image, cv::Point(map_center_x, map_center_y), radius, CV_RGB(250,0,250), -1);
  }
  ...
  void motionCommandCallback(const geometry_msgs::PoseStamped::ConstPtr& motion_command)
  {
    ...
    updateParticlesWithObservation(estimatedX, estimatedY);
